...
const json::Node JsonReader::PrintRoute(const json::Dict& request_map, RequestHandler& rh) const {
    ...
    if (!rh.IsBusNumber(route_number)) {
        ...
        result["curvature"] = rh.GetRouteInfo(route_number)->curvature;
        result["route_length"] = rh.GetRouteInfo(route_number)->route_length;
        result["stop_count"] = static_cast<int>(rh.GetRouteInfo(route_number)->stops_count);
        result["unique_stop_count"] = static_cast<int>(rh.GetRouteInfo(route_number)->unique_stops_count);
    }
...
const json::Node JsonReader::PrintStop(const json::Dict& request_map, RequestHandler& rh) const {
    ...
    if (!rh.IsStopName(stop_name)) {
        ...
        json::Array buses;
        for (auto& bus : sorted_buses_list) {
            buses.push_back(bus);
        }
        result["buses"] = buses;
    }
